          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                isActive
                  ? 'bg-[var(--rx-blue-dim)] text-[var(--rx-blue)]'
                  : 'text-[var(--rx-text-2)] hover:bg-zinc-50 hover:text-[var(--rx-text)]'
              }`
            }
          >
            {item.icon}
      ...
      <div className="px-4 py-4 border-t border-[var(--rx-border)]">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-[var(--rx-text)] truncate">{user?.name || 'Admin'}</p>
            <p className="text-[11px] text-[var(--rx-text-3)] truncate">{user?.phone}</p>
          </div>
        </div>
        <button
          onClick={handleLogout}
          className="w-full text-left px-3 py-2 rounded-lg text-xs font-semibold text-red-500 hover:bg-red-50 transition-colors flex items-center gap-2"
        >
